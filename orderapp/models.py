...
class Order(models.Model):
    ...
    assigned_to = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, limit_choices_to={
                                    'user_type': 'worker'}, related_name='assigned_orders')
    supervisor_comment = models.TextField(blank=True)
    technical_report = models.TextField(blank=True)
    completion_percentage = models.IntegerField(
        choices=COMPLETION_PERCENTAGE_CHOICES, default=0)
